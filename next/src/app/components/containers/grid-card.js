...
export const GridCard = ({ posts, url, getPostFunction }) => (
  <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 max-w-5xl gap-4 justify-items-center">
    {posts.data.map((_, index) => {
      ...
      const colorCategory = post.attributes.categories.data[0].attributes.color;

      const classBase = colors[colorCategory].bg + " bg-opacity-10";
      const classContent = colors[colorCategory].text;

      return (
        <article
          key={index}
          className="border hover:shadow-md transition-shadow cursor-pointer rounded-xl max-sm:w-80 w-[340px] h-[450px] overflow-hidden p-2 flex flex-col justify-between"
        >
            ...
            <Image
              ...
              srcSet={`${
                post.image.formats?.small?.url
                  ? `${url}${post.image.formats?.small?.url} 300w,`
                  : ""
              }`}
              sizes="100vw"
            />
