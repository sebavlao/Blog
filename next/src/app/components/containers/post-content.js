...
export const PostContent = ({post, postBody, classBase, classContent, url}) => (
    <section className="xl:w-[800px] w-full grid gap-7 max-w-[390px] sm:max-w-[660px] xl:max-w-[800px]">
        <article className="w-full grid gap-7">
            ...
            />
            <h1 className="font-semibold sm:text-xl md:text-2xl xl:text-4xl">{post.attributes.title}</h1>
            ...
            <Image
                ...
                width={"100%"}
                srcSet={
                    `${post.image.formats.small?.url ? `${url}${post.image.formats.small.url} 400w, ` : ''}` +
                    `${post.image.formats.medium?.url ? `${url}${post.image.formats.medium.url} 670w, ` : ''}` +
                    `${post.image.formats.large?.url ? `${url}${post.image.formats.large.url} 857w` : ''}`
                }
                sizes="(max-width: 400px) 200px, (max-width: 670px) 335px, 428.5px"
            />
        </article>
        <article className="text-xl px-1">
            {postBody}
        </article>
    </section>
)